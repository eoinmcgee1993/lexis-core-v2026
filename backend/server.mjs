// backend/server.mjs — LEXIS Commerce Server
...
for (const envVar of requiredEnv) {
  if (!process.env[envVar]) {
    console.error(`[LEXIS Fatal Error] Missing required environment variable: ${envVar}`);
    process.exit(1);
  }
}
...
app.get('/health', (req, res) => {
  res.json({ status: 'Operational', system: 'LEXIS Commerce v2026.3', timestamp: new Date().toISOString() });
});
