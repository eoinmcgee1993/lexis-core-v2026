    ...
    <div className="bg-white border border-lexis-ink/10 rounded-2xl overflow-hidden">
      ...
      >
        <div>
          ...
          <div className="text-xs text-lexis-ink/65 mt-0.5">{formatDate(entry.created_at)}</div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {entry.insufficient ? (
            <span className="text-[10px] uppercase tracking-wide text-lexis-ink/65">Too short to grade</span>
          ) : (
            <span className="text-sm font-display font-semibold text-teal-700">{entry.confidence}%</span>
          )}
          {hasDetail && (
            <ChevronDown className={`w-4 h-4 text-lexis-ink/65 transition-transform ${expanded ? 'rotate-180' : ''}`} aria-hidden="true" />
          )}
        </div>
      </button>
            ...
            <div className="space-y-2">
              {entry.improvements.map((imp, i) => (
                <div key={i} className="bg-lexis-canvas rounded-lg p-2.5 text-xs">
                  ...
                  {imp.note && <div className="text-lexis-ink/70 mt-1">{imp.note}</div>}
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div className="min-h-[100dvh] lexis-canvas-gradient text-lexis-ink font-sans flex flex-col">
      ...
      <div className="flex-1 w-full max-w-2xl mx-auto px-6 pb-12">
        ...
        {historyLoading && (
          <div className="flex flex-col items-center gap-3 py-12 text-lexis-ink/70">
            <Loader2 className="w-6 h-6 animate-spin" aria-hidden="true" />
          </div>
        )}

        {!historyLoading && historyError && (
          <p className="text-sm text-lexis-ink/75 py-8 text-center">Couldn't load your history right now, try again in a moment.</p>
        )}

        {!historyLoading && !historyError && history.length === 0 && (
          <p className="text-sm text-lexis-ink/70 py-8 text-center">No sessions yet. Your practice history will show up here after your first conversation.</p>
        )}
